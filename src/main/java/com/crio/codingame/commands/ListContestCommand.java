...
public class ListContestCommand implements ICommand{
    ...
    public void execute(List<String> tokens) {
        List<Contest> lContests = new ArrayList<>();
        
        if(tokens.size()==1)
            lContests = contestService.getAllContestLevelWise(null);

        else if(tokens.size()==2)
           lContests = contestService.getAllContestLevelWise(Level.valueOf(tokens.get(1)));
        
        System.out.println(lContests);
    }
